 ...
 *****************************************************************************
 *                                                                           *
 *     K0nV3rs1on3r is just a utility I think most of its functions are      *
 *     deprecated or simplified in modern java but mechanics are simple      *
 *     to visualize with the steps shown in loops                            *
 *                                                                           *
 *****************************************************************************
 */
class K0nV3rs1on3r {
 ...
 *****************************************************************************
 *                                                                           *
 *     This is where the AES happens, the "right" way to do this is to       *
 *     import javax.crypto.Cipher and make a Cipher object but with this     *
 *     more mechanical implementation we can see the S box and all the       *
 *     operations.                                                           *
 *                                                                           *
 *****************************************************************************
 */
class AES {
     ...
     *  1: trace major calls with params (setKey, encrypt, decrypt)
     *  2: + trace round values whilst en/decrypting
    ...
    public static void trace_static() {
        ...
        System.out.print("S[] = n"); for(i=0;i<16;i++) { for(j=0;j<16;j++) System.out.print(K0nV3rs1on3r.toHEX1(S[i*16+j])+", "); System.out.println();}
        System.out.print("Si[] = n"); for(i=0;i<16;i++) { for(j=0;j<16;j++) System.out.print(K0nV3rs1on3r.toHEX1(Si[i*16+j])+", "); System.out.println();}
        System.out.print("rcon[] = n"); for(i=0;i<5;i++) {for(j=0;j<6;j++) System.out.print(K0nV3rs1on3r.toHEX1(rcon[i*6+j])+", "); System.out.println();}
        System.out.print("log[] = n"); for(i=0;i<32;i++) {for(j=0;j<8;j++) System.out.print(K0nV3rs1on3r.toHEX1(log[i*8+j])+", "); System.out.println();}
        System.out.print("alog[] = n"); for(i=0;i<32;i++) {for(j=0;j<8;j++) System.out.print(K0nV3rs1on3r.toHEX1(alog[i*8+j])+", "); System.out.println();}
    }
     ...
     *
     *Follows cipher specification given in FIPS-197 section 5.1
    ...
    public byte[] encrypt(byte[] plain) {
        ...
        if (traceLevel > 0) traceInfo = "encryptAES(" + K0nV3rs1on3r.toHEX1(plain) + ")";
        ...
        if (traceLevel > 2)
            traceInfo += "n  R0 (Key = "+K0nV3rs1on3r.toHEX1(Ker)+")ntAK = "+K0nV3rs1on3r.toHEX1(a);
        else if (traceLevel > 1)
            traceInfo += "n  R0 (Key = "+K0nV3rs1on3r.toHEX1(Ker)+")t = "+K0nV3rs1on3r.toHEX1(a);

        // for each round except last, apply round transforms
        for (int r = 1; r < numRounds; r++) {
            Ker = Ke[r];            // get session keys for this round
            if (traceLevel > 1)    traceInfo += "n  R"+r+" (Key = "+K0nV3rs1on3r.toHEX1(Ker)+")t";

            // SubBytes(state) into ta using S-Box S
            for (i = 0; i < BLOCK_SIZE; i++) ta[i] = S[a[i] & 0xFF];
            if (traceLevel > 2)    traceInfo += "ntSB = "+K0nV3rs1on3r.toHEX1(ta);
            ...
            if (traceLevel > 2)    traceInfo += "ntSR = "+K0nV3rs1on3r.toHEX1(a);
        ...
        for (i = 0; i < BLOCK_SIZE; i++)    a[i] = (byte)(ta[i] ^ Ker[i]);
        if (traceLevel > 2)    traceInfo += "ntAK";
        if (traceLevel > 1)    traceInfo += " = "+K0nV3rs1on3r.toHEX1(a)+"n";
        if (traceLevel > 0)    traceInfo += " = "+K0nV3rs1on3r.toHEX1(a)+"n";
        return (a);
    }
    ...
    public byte[] decrypt(byte[] cipher) {
        ...
        traceInfo = "";                // reset trace info
        if (traceLevel > 0) traceInfo = "decryptAES(" + K0nV3rs1on3r.toHEX1(cipher) + ")";
        ...
        if (traceLevel > 2)
            traceInfo += "n  R0 (Key = "+K0nV3rs1on3r.toHEX1(Kdr)+")nt AK = "+K0nV3rs1on3r.toHEX1(a);
        else if (traceLevel > 1)
            traceInfo += "n  R0 (Key = "+K0nV3rs1on3r.toHEX1(Kdr)+")t = "+K0nV3rs1on3r.toHEX1(a);

        // for each round except last, apply round transforms
        for (int r = 1; r < numRounds; r++) {
            Kdr = Kd[r];            // get session keys for this round
            if (traceLevel > 1)    traceInfo += "n  R"+r+" (Key = "+K0nV3rs1on3r.toHEX1(Kdr)+")t";
            ...
            if (traceLevel > 2)    traceInfo += "ntISR = "+K0nV3rs1on3r.toHEX1(ta);

            // InvSubBytes(state) into a using inverse S-box Si
            for (i = 0; i < BLOCK_SIZE; i++) a[i] = Si[ta[i] & 0xFF];
            if (traceLevel > 2)    traceInfo += "ntISB = "+K0nV3rs1on3r.toHEX1(a);

            // AddRoundKey(state) into ta
            for (i = 0; i < BLOCK_SIZE; i++)    ta[i] = (byte)(a[i] ^ Kdr[i]);
            if (traceLevel > 2)    traceInfo += "nt AK = "+K0nV3rs1on3r.toHEX1(ta);
        ...
        if (traceLevel > 1)    traceInfo += "n  R"+numRounds+" (Key = "+K0nV3rs1on3r.toHEX1(Kdr)+")t";
        ...
        if (traceLevel > 2)    traceInfo += "ntISR = "+K0nV3rs1on3r.toHEX1(a);
    ...
    public void setKey(byte[] key) {
        ...
        traceInfo = "";            // reset trace info
        if (traceLevel > 0) traceInfo = "setKey(" + K0nV3rs1on3r.toHEX1(key) + ")n";

        // check for bad arguments
        if (key == null)
            throw new IllegalArgumentException("Empty key");
        if (!(key.length == 16 || key.length == 24 || key.length == 32))
            throw new IllegalArgumentException("Incorrect key length   "+ "It is  : " + key.length);
        ...
        if (traceLevel > 3) {
            ...
            for(r=0;r<numRounds+1;r++) traceInfo += "  R"+r+"t = "+K0nV3rs1on3r.toHEX1(Ke[r])+"n";
            traceInfo += "  Decrypt Round keys:n";
            for(r=0;r<numRounds+1;r++) traceInfo += "  R"+r+"t = "+K0nV3rs1on3r.toHEX1(Kd[r])+"n";
        }
    }
    ...
    public static void self_test (String hkey, String hplain, String hcipher, int lev) {
        ...
        byte [] key    = K0nV3rs1on3r.hex2byte(hkey);
        byte [] plain    = K0nV3rs1on3r.hex2byte(hplain);
        byte [] cipher    = K0nV3rs1on3r.hex2byte(hcipher);
        byte [] result;
        ...
        if (Arrays.equals(result, cipher))
            ...
            System.out.print("Test Failed. Result was "+K0nV3rs1on3r.toHEX(result)+"n");
...
public class EduCrypt {
    ...
    public static void sendString(String args, int port) throws IOException, ClassNotFoundException {
        ...
        ois.writeObject(args);
        ois.flush();
        is.close();
        socket.close();

    }
    public static void genKey(String outfile) throws IOException {
        byte[] array = new byte[32];
        new Random().nextBytes(array);
        AES aes = new AES();
        String key = aes.static_byteArrayToString(array);
        stringToFile(key, outfile);
    }

    public static void genKeyFile() throws IOException, NoSuchAlgorithmException {
        byte[] array = new byte[32];
        new Random().nextBytes(array);
        AES aes = new AES();
        String key = aes.static_byteArrayToString(array);
        String student = getString("What is the name of the student this key is for?");
        String dateString = getString("What is the date or identifier code for this key?");
        String filestring = "";
        String[] artList = new String[]{
        ...
        for (int i = 0; i < artList.length; i++) {
            filestring = filestring + artList[i] + "\n";
        }
        filestring = filestring + "Student: " + student + '\n' + dateString + '\n' + "\n#########EDUCRYPT##########\n" + key;
        String filename = "KEY-" + student + "-" + dateString + ".eduKey";
        stringToFile(filestring, filename);
    }

    public static String readKeyFile(String path) throws IOException {
        String rawfile = stringFromFile(path);


        String[] key = rawfile.split("\n#########EDUCRYPT##########\n");

        return key[1];
    }

    public static void encryptFile(String fileName, String key, String outfile) throws IOException  {

        fileName = fileName.replace("\\", "\\\\");
        //Do the size
        byte [] fileBytes = fileToBytes(fileName);
        AES aes = new AES();
        String data = aes.static_byteArrayToString(fileBytes);
        aes.setKey(key);
        data = aes.Encrypt(data);
        stringToFile(data, outfile);
    ...
    public static void stringToFile(String str, String fileName) throws IOException {
        ...
        AES aes = new AES();
        byte[] endBytes = aes.static_stringToByteArray(str);
    ...
    public static String encryptString(String data, String key) throws IOException  {
//        System.out.println(data.length());
        int padding = data.length() % 32;
//        System.out.println(padding);
        ...
        AES aes = new AES();
        aes.setKey(key);
//        byte [] byter = aes.static_stringToByteArray(data);
        String outstring = aes.Encrypt(data);
...
//    }
    public static String stringFromFile(String filePath) throws IOException {
        ...
        byte [] readBytes = fileToBytes(filePath);
        AES aes = new AES();
        String str = aes.static_byteArrayToString(readBytes);
        return str;
    }

    public static void decryptFile(String cipherPath,String key, String outfileName) throws IOException {
//        String key = stringFromFile(keyfile);
        String readBack = stringFromFile(cipherPath);
        AES aes = new AES();
        aes.setKey(key);

        String data = aes.Decrypt(readBack);
        byte [] endBytes = aes.static_stringToByteArray(data);
    ...
    public static void printCastle() throws InterruptedException {
        String[] artList = new String[]{
                "                             -|             |-                                   ",
                "         -|                  [-_-_-_-_-_-_-_-]                  |-               ",
                "         [-_-_-_-_-]          |             |          [-_-_-_-_-]               ",
    ...
    public static void printLock() throws InterruptedException {
        String[] artList = new String[]{
    ...
    public static void printKey() throws InterruptedException {
        String[] artList = new String[]{
...
//                "                                                                                             "};
    ...
    public static void printMenu() throws InterruptedException {
        System.out.println("Encryption Functions: \n");
        ...
        System.out.println("5. Generate KeyFile \n");
        System.out.println("iamdone. Quit EduCrypt \n");
    }


    public static String getString(String whatWeWant) throws NoSuchAlgorithmException, IOException {
        String ip = "wrong";
        Scanner reader = new Scanner(System.in);
        while (ip.equals("wrong")) {
            System.out.println(whatWeWant);
            ip = reader.nextLine();
            //reader.close();
        }
        return ip;
    ...
    public static String menu() throws IOException, NoSuchAlgorithmException {
        String choice = getString("Choose from amongst these options and press ENTER:");
        return choice;
...
//        String whine = getString("Let's test the functions");
//        String testkey = readKeyFile("G:\\LandOfThroth\\EduCrypt\\KEY-test-hw0.eduKey");
//        AES aes = new AES();
//        aes.setKey(testkey);
////        whine = aes.Encrypt(whine);
//
////        whine = aes.Decrypt(whine);
//        whine = encryptString(whine, testkey);
//        System.out.println(whine);
//        whine = decryptString(whine, testkey);
//
//        System.out.println(whine);
        ...
        while (going) {
            ...
            String choice = menu();
            if (choice.equals("1")) {
                String whichFile = getString("Please input the path to the file you wish to decrypt:\n");
                String keyFile = getString("Please input the path to the keyfile:\n");
                String key = readKeyFile(keyFile);
                String fileName = getString("What should we call the decrypted file?\n");
                decryptFile(whichFile, key, fileName);
                printLock();
                System.out.println("The file " + whichFile + " has been decrypted into " + fileName);
                ...
                String keyFile = getString("Please input the path to the keyfile:\n");
                String key = readKeyFile(keyFile);
                String cipher = getString("What ciphertext are we trying to decrypt?\n");
                String plainText = decryptString(cipher, key);
                ...
                String whichFile = getString("Please input the path to the file you wish to encrypt:\n");
                String keyFile = getString("Please input the path to the keyfile:\n");
                String key = readKeyFile(keyFile);
                String fileName = getString("What should we call the encrypted file?\n");
                encryptFile(whichFile, key, fileName);
                printCastle();
                System.out.println("The file " + whichFile + " has been encrypted into " + fileName);
                ...
                String keyFile = getString("Please input the path to the keyfile:\n");
                String key = readKeyFile(keyFile);
                String cipher = getString("What plainText are we trying to encrypt?\n");
                String plainText = encryptString(cipher, key);
                System.out.println(plainText);
                stringToFile(plainText, "cryptoString.txt");
            ...
            else if (choice.equals("iamdone")) {

                going = false;
            }

            else {
                System.out.println("There seems to be an issue with your input....  this is not good .... is there a chance that you did not input a single character string of a digit between 1-5?");
